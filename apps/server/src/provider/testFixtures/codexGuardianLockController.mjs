...
const pythonExecutable = process.argv[3];
const lockPeerEntry = process.argv[4];
const lockPath = process.argv[5];
if (!guardianEntry || !pythonExecutable || !lockPeerEntry || !lockPath) {
  throw new Error("guardian, Python, lock-peer, and lock paths are required");
}

const spec = JSON.stringify({
  command: pythonExecutable,
  args: [lockPeerEntry, lockPath],
  cwd: process.cwd(),
  shell: false,
